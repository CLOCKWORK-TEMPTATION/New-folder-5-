...
function extractMetaFromHtml(html: string, selector: string): string | null {
...
function buildFetchedDocument(params: {
  ...
  contentType: string;
  rawContent: string;
  retrievalMode: FetchedDocument["retrieval_mode"];
  title?: string | null;
  author?: string | null;
...
export const fetchWebpageTool = tool(
  async ({ url }: { url: string }) => {
    ...
    return JSON.stringify(
      buildFetchedDocument({
        ...
        author:
          extractMetaFromHtml(rawContent, "name=['\"]author['\"]") ??
          extractMetaFromHtml(rawContent, "property=['\"]article:author['\"]"),
        publishDate:
          extractMetaFromHtml(
            rawContent,
            "property=['\"]article:published_time['\"]",
          ) ?? extractMetaFromHtml(rawContent, "itemprop=['\"]datePublished['\"]"),
      }),
      null,
      2,
    );
  },
  {
    ...
    description:
      "Downloads raw HTML, PDF, image, or JSON from a URL and returns a structured fetched document payload.",
    schema: z.object({
      url: z.string().url(),
    }),
  },
...
export const fetchWithHeadlessBrowserTool = tool(
  async ({ url }: { url: string }) => {
  ...
  },
  {
    ...
    description:
      "Renders a page in a headless browser and returns the final HTML with structured metadata.",
    schema: z.object({
      url: z.string().url(),
    }),
  },
