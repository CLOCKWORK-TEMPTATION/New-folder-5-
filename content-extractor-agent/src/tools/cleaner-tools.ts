...
async function resolveBufferInput(input: string): Promise<Buffer> {
  const fetched = (() => {
    try {
  ...
  if (/^https?:\/\//i.test(input)) {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to download binary source: ${response.status} ${response.statusText}`,
      );
    }
...
export const extractCoreContentTool = tool(
  async ({
    rawDocument,
  }: {
    rawDocument: string;
  ...
  },
  {
    ...
    description:
      "Cleans fetched content, removes boilerplate, and produces classified semantic content blocks.",
    schema: z.object({
      rawDocument: z
        .string()
...
export const ocrVisualExtractionTool = tool(
  async ({
    documentPayload,
    pdfOrImagePath,
  }: {
    documentPayload?: string;
    pdfOrImagePath?: string;
  }) => {
    ...
    const inferredContentType =
      pdfOrImagePath?.toLowerCase().endsWith(".pdf") ||
  ...
  },
  {
    ...
    description:
      "Extracts text from PDFs or images using PDF parsing when possible and OCR otherwise.",
    schema: z.object({
