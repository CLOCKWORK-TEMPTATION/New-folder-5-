...
export function parseFetchedDocument(input: string): FetchedDocument {
  const parsed = tryParseJson<FetchedDocument>(input);
...
export function classifyBlock(text: string): ContentBlockType {
...
export function cleanFetchedDocument(input: string): CleanedDocument {
  ...
  if (!looksLikeHtml(rawContent)) {
    ...
    const contentBlocks = paragraphs.map<CleanedContentBlock>((paragraph) => ({
      type: classifyBlock(paragraph),
      text: paragraph,
      stats_source:
        classifyBlock(paragraph) === "إحصاء" ? document.source_url : null,
    }));
  ...
  $(
    '[class*="ad"], [id*="ad"], [class*="promo"], [class*="sidebar"], [class*="comment"], [class*="newsletter"], [class*="breadcrumb"]',
  ).remove();
  ...
  const contentBlocks = unique.map<CleanedContentBlock>((text) => {
...
export function normalizeCleanedDocument(input: string): UnifiedContentDocument {
  const cleaned = tryParseJson<CleanedDocument>(input) ?? cleanFetchedDocument(input);
  const hostname =
    cleaned.source_url && cleaned.source_url !== "unknown"
      ? new URL(cleaned.source_url).hostname.toLowerCase()
      : "";

  let source_type: UnifiedContentDocument["source_type"] = "مدونة";
  let credibility_tier: UnifiedContentDocument["credibility_tier"] = 3;
  ...
  const direct_quote_available = cleaned.content_blocks.some((block) =>
    /["'«»“”]/.test(block.text),
  );
  ...
  return {
    ...
    content_blocks: cleaned.content_blocks.map((block) => ({
      ...
      text: block.text,
      stats_source:
        block.type === "إحصاء" ? block.stats_source ?? cleaned.source_url : null,
    })),
  };
}

export function parsePossiblyStructuredInput(input: string): string {
  const parsedFetched = tryParseJson<FetchedDocument>(input);
  if (parsedFetched?.raw_content) {
    return parsedFetched.raw_content;
  }

  const parsedCleaned = tryParseJson<CleanedDocument>(input);
