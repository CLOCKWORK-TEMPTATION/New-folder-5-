...
import { createDeepAgent, StateBackend, StoreBackend, FilesystemBackend, LocalShellBackend, } from "deepagents";
...
import { z } from "zod";
import { queryGeneratorAgent, searchExecutorAgent, resultRankerAgent, } from "./subagents/index.js";
 ...
 */
const validateSearchPlan = tool(async ({ planJson }) => {
    try {
        const plan = JSON.parse(planJson);
        const issues = [];
        if (!plan.topic?.trim())
            issues.push("الموضوع مطلوب");
        if (!plan.objective?.trim())
            issues.push("الهدف مطلوب");
        if (!plan.languages?.length)
            issues.push("يجب تحديد لغة واحدة على الأقل");
        if (!plan.engines?.length)
            issues.push("يجب تحديد محرك بحث واحد على الأقل");
        ...
        const invalidEngines = (plan.engines || []).filter((e) => !validEngines.includes(e));
 ...
 */
const assembleSearchOutput = tool(async ({ queryResultsJson, searchResultsJson, rankingResultsJson, enginesUsed, languagesSearched, }) => {
 ...
 */
function createBackend(type, rootDir) {
    switch (type) {
        case "store":
            return (config) => new StoreBackend(config);
        case "filesystem":
            return (config) => new FilesystemBackend({ rootDir: rootDir || "./search-workspace" });
        case "local-shell":
        ...
        case "state":
        default:
            return (config) => new StateBackend(config);
    }
}
// ─────────────────────────────────────────────
 ...
 * - Model override لكل وكيل فرعي
 */
export function createSearchScoutAgent(config) {
 ...
 */
export async function executeSearch(plan, config) {
    ...
    const invokeConfig = config?.threadId
        ...
        : undefined;
    const result = await agent.invoke({ messages: [{ role: "user", content: prompt }] }, invokeConfig);
    return {
 ...
 */
function buildSearchPrompt(plan) {
...
async function main() {
    ...
    const samplePlan = {
        ...
        objective: "جمع أحدث الدراسات والتقارير حول استخدام الذكاء الاصطناعي التوليدي في التعليم العربي، مع التركيز على التجارب الفعلية والتحديات",
        ...
        additionalInstructions: "ركّز على المصادر الأكاديمية والتقارير المؤسسية. تجنب المدونات الشخصية والمحتوى التسويقي.",
    };
    try {
