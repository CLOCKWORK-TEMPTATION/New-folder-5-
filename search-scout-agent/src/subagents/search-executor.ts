 ...
 */
export function createSearchExecutorAgent(modelOverride?: string): SubAgent {
  return {
    name: "search-executor",
    description:
      "ينفذ استعلامات البحث بالتوازي على محركات متعددة (Serper, Google, Bing, Tavily). يدير التوازي وrate limits ومعالجة الأخطاء تلقائياً. يجمع النتائج الخام مع إحصائيات التنفيذ. استخدمه بعد توليد الاستعلامات لتنفيذ البحث الفعلي.",
...
export const searchExecutorAgent: SubAgent = createSearchExecutorAgent();
