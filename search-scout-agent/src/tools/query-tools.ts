 ...
 */
export const expandSearchTopic = tool(
  async ({
    topic,
    objective,
    seedKeywords,
  ...
  },
  {
    ...
    description:
      "تحليل موضوع البحث وتوليد زوايا ومنظورات بحثية متعددة. تستكشف هذه الأداة المرادفات والمفاهيم ذات الصلة والصياغات المختلفة للموضوع لضمان تغطية شاملة قبل توليد الاستعلامات الفعلية.",
    schema: z.object({
      topic: z
        .string()
        .min(1)
      ...
      seedKeywords: z
        ...
        .default([])
        .describe(
          "الكلمات المفتاحية الأولية كنقطة انطلاق لتوسيع الموضوع"
        ),
    }),
  }
);
 ...
 */
export const generateSearchQueries = tool(
  async ({
    anglesJson,
    languages,
    targetEngines,
    ...
    const engineOperators: Record<
      string,
      { supportsQuotes: boolean; supportsBoolean: boolean; siteFilter: string }
    > = {
      google: {
        supportsQuotes: true,
  ...
  },
  {
    ...
    description:
      "توليد نصوص استعلامات البحث المُحسَّنة لكل تركيبة من محرك ولغة. تأخذ الأداة الزوايا البحثية الناتجة عن expandSearchTopic وتُنتج استعلامات جاهزة للتنفيذ مع مراعاة خصائص كل محرك (المشغّلات المنطقية، الاقتباسات) ومتطلبات كل لغة.",
    schema: z.object({
      anglesJson: z
        .string()
        .min(2)
        .describe(
          "مصفوفة JSON للزوايا البحثية الناتجة عن expandSearchTopic أو كائن يحتوي على حقل angles"
        ),
      languages: z
        .array(z.string())
        .min(1)
        .describe(
          'مصفوفة رموز اللغات المطلوبة مثل ["ar", "en", "fr"]'
        ),
      targetEngines: z
        .array(z.string())
        .min(1)
        .describe(
          'مصفوفة أسماء محركات البحث المستهدفة مثل ["google", "bing", "tavily"]'
        ),
    }),
  }
);
 ...
 */
export const buildQueryStrategy = tool(
  async ({
    queriesJson,
    maxResultsPerEngine,
    excludedDomains,
    ...
    const buildExclusionFilter = (engine: string): string => {
      if (!excludedDomains || excludedDomains.length === 0) return "";
      const supportsNegation = ["google", "bing"].includes(engine);
      if (!supportsNegation) return "";
    ...
    const totalBatches = executionPlan.reduce(
      (sum, ep) => sum + ep.batches.length,
      0
    );
    const totalEstimatedResults = executionPlan.reduce(
      (sum, ep) => sum + ep.totalEstimatedResults,
      0
    );
  ...
  },
  {
    ...
    description:
      "بناء خطة تنفيذ الاستعلامات النهائية وتوزيعها على محركات البحث. تُجمِّع الأداة الاستعلامات في دُفعات مُرتَّبة حسب الأولوية لكل محرك، وتُعالج النطاقات المستبعدة، وتُنتج خطة تنفيذ شاملة مع تقديرات للنتائج المتوقعة.",
    schema: z.object({
      queriesJson: z
        .string()
        .min(2)
        .describe(
          "مصفوفة JSON للاستعلامات الناتجة عن generateSearchQueries أو كائن يحتوي على حقل queries"
        ),
      maxResultsPerEngine: z
        ...
        .default(10)
        .describe(
          "الحد الأقصى لعدد النتائج المطلوبة من كل محرك بحث"
        ),
      excludedDomains: z
        .array(z.string())
        .optional()
        .describe(
          'قائمة النطاقات المستبعدة من نتائج البحث مثل ["example.com", "spam.net"]'
        ),
    }),
  }
);
