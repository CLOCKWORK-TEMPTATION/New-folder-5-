...
import { z } from "zod";
import type { RawSearchResult, RankedSearchResult } from "../types/index.js";
 ...
 */
function getDomainAuthorityBonus(domain: string): number {
  if (domain.endsWith(".edu")) return 15;
  ...
  if (trustedDomains.some((d) => domain.endsWith(d))) return 5;
  return 0;
}
 ...
 */
function computeTextRelevance(text: string, topic: string): number {
 ...
 */
function computeRecencyScore(publishDate?: string): number {
  if (!publishDate) return 5; // قيمة افتراضية محايدة
...
export const deduplicateResults = tool(
  async ({ rawResultsJson }: { rawResultsJson: string }) => {
    ...
    try {
      ...
      return JSON.stringify({
        error: `فشل تحليل نتائج البحث الخام: ${(err as Error).message}`,
        deduplicated: [],
    ...
    const mergeMap = new Map<
      string,
      {
        representative: RawSearchResult;
  ...
  },
  {
    ...
    description:
      "يزيل التكرارات من نتائج البحث الخام القادمة من محركات متعددة. " +
    ...
    schema: z.object({
      rawResultsJson: z
        .string()
        .describe(
          "مصفوفة JSON من نتائج البحث الخام (RawSearchResult[]). " +
...
export const scoreAndRankResults = tool(
  async ({
    ...
    try {
      ...
      return JSON.stringify({
        error: `فشل تحليل النتائج المُزالة تكراراتها: ${(err as Error).message}`,
        scored: [],
      });
    }
    ...
    const scored: RankedSearchResult[] = results.map((result) => {
      ...
      const snippetRelevance = computeTextRelevance(
        result.snippet,
        searchTopic
      );
      ...
      if (crossScore > 0) {
        reasonParts.push(
          `ظهر في ${result.crossEngineCount} محركات: +${crossScore}`
        );
      }
      ...
      const totalScore = Math.min(
        100,
        titleScore + snippetScore + authorityScore + crossScore + recencyScore
      );
      ...
      return {
        ...
        foundInEngines: result.foundInEngines as RankedSearchResult["foundInEngines"],
    ...
    return JSON.stringify({
      ...
      scoreDistribution: {
        ...
        medium: scored.filter(
          (r) => r.relevanceScore >= 40 && r.relevanceScore < 70
        ).length,
  ...
  },
  {
    ...
    description:
      "يُسجِّل النقاط لكل نتيجة بحث ويُرتِّبها تنازليًا حسب الصلة. " +
    ...
    schema: z.object({
      deduplicatedResultsJson: z
        .string()
        .describe(
          "مخرج أداة deduplicateResults بصيغة JSON. " +
            "يجب أن يحتوي على حقل 'deduplicated' وهو مصفوفة النتائج."
        ),
      searchTopic: z
        .string()
        .describe(
          "موضوع البحث أو هدفه بصياغة واضحة تُستخدم لحساب درجة الصلة. " +
...
export const filterAndFinalize = tool(
  async ({
    ...
    try {
      ...
      return JSON.stringify({
        error: `فشل تحليل النتائج المُسجَّلة: ${(err as Error).message}`,
        rankedResults: [],
        totalBeforeFilter: 0,
        totalAfterFilter: 0,
        duplicatesRemoved: 0,
    ...
    if (excludedDomainsJson) {
      try {
        ...
        if (Array.isArray(parsed)) {
          excludedDomains = parsed.map((d: string) =>
    ...
    // ─── توليد ملخص التغطية ───
    const coverageSummary = generateCoverageSummary(
      cappedResults,
      totalBeforeFilter,
      excludedDomains
    );
      ...
      metadata: {
        ...
        averageRelevanceScore:
          cappedResults.length > 0
            ? Math.round(
                cappedResults.reduce((sum, r) => sum + r.relevanceScore, 0) /
  ...
  },
  {
    ...
    description:
      "تُطبِّق الفلترة النهائية على النتائج المُرتَّبة: " +
    ...
    schema: z.object({
      scoredResultsJson: z
        .string()
        .describe(
          "مخرج أداة scoreAndRankResults بصيغة JSON. " +
      ...
      maxResults: z
        ...
        .positive()
        .describe(
          "الحد الأقصى لعدد النتائج المُعادة في القائمة النهائية."
        ),
      excludedDomainsJson: z
        .string()
        .optional()
        .describe(
          "مصفوفة JSON اختيارية من النطاقات المستبعدة. " +
...
function generateCoverageSummary(
  ...
  // ─── إحصائيات عامة ───
  parts.push(
    `تم تحليل ${totalBeforeFilter} نتيجة وتصفيتها إلى ${results.length} نتيجة نهائية.`
  );
  ...
  if (multiEngine > 0) {
    parts.push(
      `${multiEngine} نتيجة ظهرت في أكثر من محرك بحث واحد مما يعزز موثوقيتها.`
    );
  }
  ...
  if (authoritative.length > 0) {
    parts.push(
      `${authoritative.length} نتيجة من مصادر ذات سلطة (.edu / .gov / .org).`
    );
  }
  ...
  if (withDates.length > 0) {
    ...
    parts.push(
      "تحذير: لا تتوفر تواريخ نشر للنتائج، مما يُصعِّب تقييم حداثة المحتوى."
    );
  }
  ...
  if (excludedDomains.length > 0) {
    parts.push(
      `تم استبعاد النطاقات التالية وفق إعدادات الفلترة: ${excludedDomains.join("، ")}.`
    );
  }
  ...
  if (lowScoreCount > results.length * 0.5) {
    parts.push(
      "ملاحظة: أكثر من نصف النتائج ذات صلة منخفضة، يُنصح بتوسيع استعلامات البحث."
    );
  }

  if (uniqueDomains < 3) {
    parts.push(
      "ملاحظة: التنوع في المصادر محدود، قد يكون هناك زوايا ومنظورات غير مُغطاة."
    );
  }
