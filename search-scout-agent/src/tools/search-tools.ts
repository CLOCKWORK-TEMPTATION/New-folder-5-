...
import type { RawSearchResult, SearchEngine } from "../types/index.js";
...
export const serperSearch = tool(
  async ({
    ...
    if (!apiKey) {
      throw new Error(
        "مفتاح SERPER_API_KEY غير موجود في متغيرات البيئة. " +
    ...
    if (gl) requestBody.gl = gl;
    if (hl) requestBody.hl = hl;
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "خطأ غير معروف في الاتصال";
      if ((err as { name?: string }).name === "AbortError") {
        throw new Error(
          `انتهت مهلة طلب Serper بعد ${DEFAULT_TIMEOUT_MS} مللي ثانية. ` +
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `رد Serper غير ناجح: HTTP ${response.status} ${response.statusText}. ` +
          `التفاصيل: ${errorText}`
      );
    }
    ...
    const results: RawSearchResult[] = rawItems
      .slice(0, maxResults)
      .map((item, idx) => {
        const url =
          (searchType === "images"
        ...
        return {
          ...
          snippet:
            ...
            "",
          engine: "serper" as SearchEngine,
          query,
          position: item.position ?? idx + 1,
          domain: extractDomain(url),
          publishDate: (item as { date?: string }).date,
        };
      });
  ...
  },
  {
    name: "serper_search",
    description:
      "أداة بحث Serper: تُنفّذ بحثاً عبر واجهة برمجة Serper التي توفر نتائج Google. " +
    ...
    schema: z.object({
      ...
      searchType: z
        ...
        .default("search")
        .describe(
          "نوع البحث: search للبحث العادي، news للأخبار، images للصور"
        ),
      gl: z
        .string()
        .optional()
...
export const googleSearch = tool(
  async ({
    ...
    if (!apiKey) {
      throw new Error(
        "مفتاح GOOGLE_SEARCH_API_KEY غير موجود في متغيرات البيئة. " +
          "يرجى إنشاء مفتاح API من Google Cloud Console وتعيينه."
      );
    }
    if (!engineId) {
      throw new Error(
        "معرّف GOOGLE_SEARCH_ENGINE_ID غير موجود في متغيرات البيئة. " +
    ...
    for (let page = 0; page < totalPages; page++) {
      ...
      try {
        ...
        if ((err as { name?: string }).name === "AbortError") {
          throw new Error(
            `انتهت مهلة طلب Google Custom Search (صفحة ${page + 1}) ` +
              `بعد ${DEFAULT_TIMEOUT_MS} مللي ثانية.`
          );
        }
        throw new Error(
          `فشل طلب Google Custom Search بسبب خطأ في الشبكة: ${message}`
        );
      } finally {
        clear();
      }
      ...
      const pageResults: RawSearchResult[] = items.map((item, idx) => {
        const url = item.link ?? "";
  ...
  },
  {
    name: "google_search",
    description:
      "أداة بحث Google Custom Search: تُنفّذ بحثاً عبر Google Custom Search JSON API. " +
    ...
    schema: z.object({
      ...
      language: z
        ...
        .optional()
        .describe(
          "رمز اللغة لتصفية النتائج مثل ar أو en (يُضاف lang_ تلقائياً)"
        ),
      dateRestrict: z
        .string()
        .optional()
        .describe(
          "تقييد النتائج حسب التاريخ مثل d7 لآخر 7 أيام أو m1 لآخر شهر أو y1 لآخر سنة"
        ),
    }),
  }
);
...
export const bingSearch = tool(
  async ({
    ...
    if (!apiKey) {
      throw new Error(
        "مفتاح BING_SEARCH_API_KEY غير موجود في متغيرات البيئة. " +
    ...
    while (allResults.length < totalToFetch) {
      ...
      try {
        ...
        if ((err as { name?: string }).name === "AbortError") {
          throw new Error(
            `انتهت مهلة طلب Bing Search بعد ${DEFAULT_TIMEOUT_MS} مللي ثانية.`
          );
        }
        throw new Error(
          `فشل طلب Bing Web Search بسبب خطأ في الشبكة: ${message}`
        );
      } finally {
        clear();
      }
  ...
  },
  {
    name: "bing_search",
    description:
      "أداة بحث Bing Web Search: تُنفّذ بحثاً عبر Microsoft Bing Web Search API v7. " +
    ...
    schema: z.object({
      ...
      market: z
        ...
        .optional()
        .describe(
          "رمز السوق الجغرافي واللغوي مثل ar-SA للسعودية أو en-US للولايات المتحدة"
        ),
      freshness: z
        .enum(["Day", "Week", "Month"])
        .optional()
        .describe(
          "تصفية النتائج حسب الحداثة: Day لآخر يوم، Week لآخر أسبوع، Month لآخر شهر"
        ),
    }),
  }
);
...
export const tavilySearch = tool(
  async ({
    ...
    if (!apiKey) {
      throw new Error(
        "مفتاح TAVILY_API_KEY غير موجود في متغيرات البيئة. " +
    ...
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) =>
        setTimeout(
          () =>
            reject(
              new Error(
                `انتهت مهلة طلب Tavily بعد ${DEFAULT_TIMEOUT_MS} مللي ثانية.`
              )
            ),
          DEFAULT_TIMEOUT_MS
        )
      ...
      const message =
        err instanceof Error ? err.message : "خطأ غير معروف في Tavily";
    ...
    // تحويل النتائج إلى الصيغة المشتركة
    const results: RawSearchResult[] = parsedResults
      .slice(0, maxResults)
      .map((item, idx) => {
        const url = item.url ?? "";
        return {
  ...
  },
  {
    name: "tavily_search",
    description:
      "أداة بحث Tavily: تُنفّذ بحثاً ذكياً مخصصاً للذكاء الاصطناعي عبر Tavily API. " +
    ...
    schema: z.object({
      ...
      topic: z
        ...
        .default("general")
        .describe(
          "موضوع البحث: general للبحث العام، news للأخبار، finance للمالية"
        ),
      includeRawContent: z
        .boolean()
        .default(false)
...
// ─────────────────────────────────────────────
// 5. أداة تنفيذ البحث المتوازي
// ─────────────────────────────────────────────

/**
 * واجهة داخلية لطلب البحث الفردي ضمن executeParallelSearch
 */
...
/**
 * إحصائيات محرك بحث من تنفيذ متوازٍ
 */
  ...
  try {
    switch (engine) {
      ...
      default: {
        return {
          results: [],
          error: `محرك بحث غير مدعوم: ${engine as string}`,
        };
      }
    }
  } catch (err: unknown) {
    const message =
      err instanceof Error ? err.message : "خطأ غير معروف أثناء التنفيذ";
 ...
 */
export const executeParallelSearch = tool(
  async ({ queries }: { queries: string }): Promise<{
    results: RawSearchResult[];
    totalResults: number;
    engineStats: EngineStats[];
    failedQueries: Array<{ query: string; engine: SearchEngine; error: string }>;
    summary: string;
  }> => {
    // تحليل مصفوفة الاستعلامات من JSON
    let searchRequests: ParallelSearchRequest[];
    try {
      searchRequests = JSON.parse(queries) as ParallelSearchRequest[];
    } catch {
      throw new Error(
        "تنسيق معامل queries غير صحيح. يجب أن يكون JSON صالحاً " +
    ...
    if (!Array.isArray(searchRequests) || searchRequests.length === 0) {
      throw new Error(
        "يجب أن يحتوي معامل queries على مصفوفة غير فارغة من طلبات البحث."
      );
    }

    // التحقق من صحة كل طلب
    const validEngines: SearchEngine[] = ["serper", "google", "bing", "tavily"];
    searchRequests.forEach((req, idx) => {
      if (!req.query || typeof req.query !== "string") {
        throw new Error(
          `الطلب رقم ${idx + 1}: يجب أن يحتوي على حقل query نصي غير فارغ.`
        );
      }
      if (!validEngines.includes(req.engine)) {
        throw new Error(
          `الطلب رقم ${idx + 1}: محرك البحث "${req.engine}" غير مدعوم. ` +
    ...
    const statsMap = new Map<SearchEngine, EngineStats>();
    const initStats = (engine: SearchEngine): EngineStats => ({
      engine,
      queriesAttempted: 0,
      queriesSucceeded: 0,
      resultsReturned: 0,
    ...
    const searchPromises = searchRequests.map(async (req) => {
      ...
      const { results, error } = await dispatchToEngine(
        engine,
        req.query,
        maxResults
      );
    ...
    const summary =
      `تم تنفيذ ${totalAttempted} طلب بحث عبر ${engineStats.length} محرك. ` +
  ...
  },
  {
    ...
    description:
      "أداة تنفيذ البحث المتوازي: تستقبل قائمة من طلبات البحث (JSON) وتُنفّذها " +
    ...
    schema: z.object({
      queries: z
        .string()
        .describe(
          "JSON string يحتوي على مصفوفة من طلبات البحث. " +
